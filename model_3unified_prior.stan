...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      ...
      mu[n] <- (1 / (1 + exp(-(a[2] + b[2] * x[n]))))*(1-(heavy / (1 + exp(-(a[3] + b[3] * x[n])))));
    }
    else if (cond[n] == 3) {
      mu[n] <- (1 / (1 + exp(-(a[4] + b[3] * x[n]))));
    }
    else if (cond[n] == 4) {
      mu[n] <- (1 / (1 + exp(-(a[5] + b[4] * x[n]))));
    }
    else if (cond[n] == 5) {
      mu[n] <- (1 / (1 + exp(-(a[6] + b[5] * x[n]))));
    }
...
model {
  ...
  b[3] ~ normal(bMu[4], bSigma[4]) T[0,10];
  b[4] ~ normal(bMu[5], bSigma[5]) T[0,10];
  b[5] ~ normal(bMu[6], bSigma[6]) T[0,10];
