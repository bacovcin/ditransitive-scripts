...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      mu[n] <- ((1 - ((1 - order) * iodo[n]))/ (1 + exp(-(aToAD + bTo * x[n] + gTo * iosum[n]))));
    }
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bTo * x[n] + gTo * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bAccAct * x[n] + gAccAct * iosum[n])))));
    }
    if (cond[n] == 3) {
      mu[n] <- (donoun[n] / (1 + exp(-(aAccPas + bAccPas * x[n] + gAccPas * iosum[n]))));
    }
    if (cond[n] == 4) {
      mu[n] <- ((1 - ((1 - order) * iopro[n]))/ (1 + exp(-(aToPas + bTo * x[n] + gTo * iosum[n] ))));
    }
...
model {
  ...
  bTo ~ normal((bMuToAD+bMuToDA+bMuToPas)/3, (bSigmaToAD+bSigmaToDA+bMuToPas)/3) T[0,10];
  bAccAct ~ normal(bMuAccAct, bSigmaAccAct) T[0,10];
  bAccPas ~ normal(bMuAccPas, bSigmaAccPas) T[0,10];

  gTo ~ normal((gMuToAD+gMuToDA+gMuToPas)/3, (gSigmaToAD+gSigmaToDA+gSigmaToPas)/3) T[-5,5];
