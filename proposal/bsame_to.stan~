...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      mu[n] <- ((1 - ((1 - order) * iodo[n]))/ (1 + exp(-(aToAD + bTo * x[n] + gToAD * iosum[n]))));
    }
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bTo * x[n] + gToDA * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bAccAct * x[n] + gAccAct * iosum[n])))));
    }
    if (cond[n] == 3) {
      mu[n] <- (donoun[n] / (1 + exp(-(aAccPas + bAccPas * x[n] + gAccPas * iosum[n]))));
    }
    if (cond[n] == 4) {
      mu[n] <- ((1 - ((1 - order) * iopro[n]))/ (1 + exp(-(aToPas + bTo * x[n] + gToPas * iosum[n] ))));
    }
