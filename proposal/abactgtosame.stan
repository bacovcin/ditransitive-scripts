...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      mu[n] <- ((1 - ((1 - order) * iodo[n]))/ (1 + exp(-(aToAD + bToAct * x[n] + gTo * iosum[n]))));
    }
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bToAct * x[n] + gTo * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bAcc * x[n] + gAccAct * iosum[n])))));
    }
    if (cond[n] == 3) {
      mu[n] <- (donoun[n] / (1 + exp(-(aAccPas + bAcc * x[n] + gAccPas * iosum[n]))));
    }
    if (cond[n] == 4) {
      mu[n] <- ((1 - ((1 - order) * iopro[n]))/ (1 + exp(-(aToPas + bToPas * x[n] + gTo * iosum[n] ))));
    }
...
model {
  ...
  gTo ~ normal((gMuToAD+gMuToPas)/2, (gSigmaToAD+gSigmaToPas)/2) T[-5,5];
