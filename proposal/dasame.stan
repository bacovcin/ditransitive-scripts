...
transformed parameters {
  ...
  for (n in 1:N) {
    ...
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bDA * x[n] + gDA * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bDA * x[n] + gDA * iosum[n])))));
    }
    if (cond[n] == 3) {
...
model {
  ...
  bToAD ~ normal(bMuToAD, bSigmaToAD) T[0,10];
  bDA ~ normal((bMuToDA+bMuAccAct)/2, (bSigmaToDA+bSigmaAccAct)/2) T[0,10];
  ...
  gDA ~ normal((gMuToDA+gMuAccAct)/2, (gSigmaToDA+gSigmaAccAct)/2) T[-5,5];
