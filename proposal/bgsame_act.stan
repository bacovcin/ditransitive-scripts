...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      mu[n] <- ((1 - ((1 - order) * iodo[n]))/ (1 + exp(-(aToAD + bToAct * x[n] + gToAct * iosum[n]))));
    }
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bToAct * x[n] + gToAct * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bAccAct * x[n] + gAccAct * iosum[n])))));
    }
    if (cond[n] == 3) {
      mu[n] <- (donoun[n] / (1 + exp(-(aAccPas + bAccPas * x[n] + gAccPas * iosum[n]))));
    }
    if (cond[n] == 4) {
      mu[n] <- ((1 - ((1 - order) * iopro[n]))/ (1 + exp(-(aToPas + bToPas * x[n] + gToPas * iosum[n] ))));
    }
...
model {
  ...
  bToAct ~ normal((bMuToAD+bMuToDA)/2, (bSigmaToAD+bSigmaToDA)/2) T[0,10];
  bAccAct ~ normal(bMuAccAct, bSigmaAccAct) T[0,10];
  bAccPas ~ normal(bMuAccPas, bSigmaAccPas) T[0,10];
  bToPas ~ normal(bMuToPas, bSigmaToPas) T[0,10];

  gToAct ~ normal((gMuToAD+gMuToDA)/2, (gSigmaToAD+gSigmaToDA)/2) T[-5,5];
