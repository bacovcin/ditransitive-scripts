...
transformed parameters {
  ...
  for (n in 1:N) {
    if (cond[n] == 1) {
      mu[n] <- ((1 - ((1 - order) * iodo[n]))/ (1 + exp(-(aToAD + bTo * x[n] + gToAct * iosum[n]))));
    }
    if (cond[n] == 2) {
      mu[n] <- (((1 - ((1 - order) * iopro[n])) * donoun[n])/ (1 + exp(-(aToDA + bTo * x[n] + gToAct * iosum[n])))) * (1 - (heavy / (1 + exp(-(aAccAct + bAcc * x[n] + gAccAct * iosum[n])))));
    }
    if (cond[n] == 3) {
...
model {
  ...
  bTo ~ normal((bMuToAD+bMuToPas)/2, (bSigmaToAD+bSigmaToPas)/2) T[0,10];
